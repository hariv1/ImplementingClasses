...
class Point(object):
    ...
    def get_distance_from(self, point):
        change_x = (self.x - point.x) ** 2
        change_y = (self.y - point.y) ** 2
    ...
    def get_distance_from_start(self):
        change_x = (self.x - self.old_x) ** 2
        change_y = (self.y - self.old_y) ** 2
        return math.sqrt(change_y + change_x)
    ...
    def closer_to(self,p2,p3):
        distance_p2 = self.get_distance_from(p2)
        distance_p3 = self.get_distance_from(p3)

        if distance_p2 < distance_p3:
            return p2
        elif distance_p3 < distance_p2:
            return p3
        else:
            return p2
